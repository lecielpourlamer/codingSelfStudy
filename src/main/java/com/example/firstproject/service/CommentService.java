...
//        for (int i = 0; i < comments.size(); i++) { // 조회한 댓글 엔티티 수만큰 반복하기
...
//        }
        // 조회한 댓글 엔티티 하나씩 가져오기 enhanced for loop
//        for (Comment c : comments) { // 조회한 댓글 엔티티 수만큰 반복하기
//            CommentDto dto = CommentDto.createCommentDto(c);    // 엔티티를 DTO로 변환
//            dtos.add(dto);  // 변환한 DTO를 dtos 리스트에 삽입
//        }
        // 3. 결과 반환
...
//                .collect(Collectors.toList());    // 스트림을 리스트로 변환
    ...
    public CommentDto update(Long id, CommentDto dto) {
        // 1. DB에 해당 댓글을 조회해 가져오고 없을 경우 예외 발생시키기
